...
def find_closest(location, centroids):
    ...
    return min([k for k in centroids], key = lambda x :distance(location, x))
    # END Question 3
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    return max([find_predictor(user, reviewed, x) for x in feature_fns], key = lambda x: x[1])[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    def rate(x):
        if x in reviewed:
            return user_rating(user,restaurant_name(x))
        else:
            return predictor(x)
    return {restaurant_name(x): rate(x) for x in restaurants}
    # END Question 9
