...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        dist = 0
        while dist < self.min_range:
            current_place = current_place.entrance
            dist += 1

        while not current_place.bees and current_place.entrance != hive and dist < self.max_range:
            current_place = current_place.entrance
            dist += 1

        if not current_place.bees:
            return None
        else:
...
# END Problem 13
    ...
    def __init__(self):
        ...
        if QueenAnt.object_called == 0:
            self.true_queen = True
        QueenAnt.object_called +=1
            ...
            while current_place:
                if current_place.ant:
                    if isinstance(current_place.ant, BodyguardAnt):
                        if current_place.ant.ant is not None and current_place.ant.ant.doubled == False:
                            current_place.ant.ant.damage *= 2
                            current_place.ant.ant.doubled = True
                        if isinstance(current_place.ant,TankAnt) and current_place.ant.doubled == False:
                            current_place.ant.damage *= 2
                            current_place.ant.doubled = True
                    else:
                        if current_place.ant.doubled == False:
                            current_place.ant.damage *= 2
                            current_place.ant.doubled = True
    ...
    def reduce_armor(self, amount):
        ...
        if self.true_queen and self.armor <= 0:
            return bees_win()
        elif not self.true_queen and self.armor <= 0:
            self.place.remove_insect(self)
