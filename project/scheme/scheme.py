...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(scheme_eval(first, env), MacroProcedure):
            expr = complete_eval(scheme_apply(procedure, rest, env))
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        list_args = operands.map(lambda x: scheme_eval(x, env))
        return scheme_apply(self, list_args, env)
        # END PROBLEM 5
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        if not scheme_symbolp(bindings.first.first):
            raise SchemeError
        child.define(bindings.first.first, scheme_eval(bindings.first.second.first, env))
